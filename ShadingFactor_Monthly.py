...
def shadingFactor_Monthly(fnameRad, fileRad, climate, epw, detailedRadiationAnalysisMonthlyPath, baseFolderPath):
...
# %% Compute shadings 
    ...
    for file in os.listdir(baseFolderPath):
        if file.endswith(".csv") and "Shoebox" in file and "Monthly" in file:
    ...
    try:
        ...
        Output[0].Reporting_Frequency = "Monthly"
        idf1.run(output_prefix = "ShoeboxRadiation_" + climate + "_Monthly", readvars=True)
        ...
        for file in os.listdir(baseFolderPath):
            if os.path.isfile(file):
                if climate in file and "Monthly" in file:
                    os.rename(file, "ShoeboxRadiation_" + climate + "_Monthly.csv")
        # postprocess results to make them ready to be used
        radiationFileToModify = pd.read_csv("ShoeboxRadiation_" + climate + "_Monthly.csv")
        ...
        shoeboxRadiation.to_csv("ShoeboxRadiation_" + climate + "_Monthly.csv")
    ...
    shadingFactor.to_csv(detailedRadiationAnalysisMonthlyPath + fileRad.split(".")[0] + ".csv")